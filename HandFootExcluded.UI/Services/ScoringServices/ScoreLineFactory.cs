...
internal sealed class ScoreLineFactory : IScoreLineFactory
{
    ...
    public IScoreLines Create(IGame game, IEnumerable<IRoundViewModel> rounds, int bonusAmount)
    {
        ...
        _players = game.OrderedPlayers.Select(p => p.Initials)
                       .ToList();
        _roundOrders = game.Rounds.Select(r => r.Order)
                           .ToList();
    ...
    private static IScoreLines CreatePlayerScores(IRoundViewModel round, int bonusAmount)
    {
        ...
        return new ScoreLines().Add(startingTeamScoreLines)
                               .Add(opposingTeamScoreLines)
                               .Add(excludedPlayerScoreLines);
    }
    ...
    private static IScoreLines CreateTeamScores(IRoundViewModel round, ITeamViewModel team, int bonusAmount)
    {
        ...
        return new ScoreLines().Add(playerScoreLines)
                               .Add(partnerScoreLines);
    }
    ...
    private static IScoreLines CreateRoundTotalScore(IRoundViewModel round, ITeamViewModel team, string initials, bool hasBonus, int bonusAmount)
    {
        ...
        return new ScoreLines().Add(bonus)
                               .Add(top)
                               .Add(bottom)
                               .Add(roundTotal);
    }

    private static IRoundTotalScoreLine CreateRoundTotal(string initials, int roundOrder, IBonusScoreLine bonusScoreLine, ITopScoreLine topScoreLine, IBottomScoreLine bottomScoreLine) => new RoundTotalScoreLine(roundOrder, initials, bonusScoreLine, topScoreLine, bottomScoreLine);
    ...
    private static ICumulativeScoreLine GetCumulativeScoreLines(IScoreLines scoreLines, string player) =>
        scoreLines.OfType<ICumulativeScoreLine>()
                  .Last(l => l.Initials.Equals(player));
}